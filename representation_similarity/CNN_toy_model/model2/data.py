"""Training data for Model 2 (copied verbatim from ../model1/data.py -- same
self-containment convention as net.py): one named GoL pattern per grid,
randomly D4-transformed and placed with a margin from the torus edge (so a
single step never wraps and the pattern's bounding box is unambiguous), or
a random-density grid. Model 2 trains on the mixed distribution from the
start (make_mixed_batch) -- no need to repeat Model 1's v1-only-sparse-data
mistake.
