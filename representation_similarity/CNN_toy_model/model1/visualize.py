"""Visual demos of the trained Model 1: autoregressive rollout accuracy and
D4 equivariance, as GIFs.

Run: python visualize.py
Outputs -> results/gifs/*.gif
"""
...
def rollout(model, g0: np.ndarray, steps: int):
    """Returns (truth_frames, model_frames): lists of (GRID,GRID) arrays,
    length steps+1. Model rolls out AUTOREGRESSIVELY (feeds its own
    predictions back in), truth uses the exact toroidal GoL rule."""
...
def save_truth_vs_model_gif(name: str, g0: np.ndarray, steps: int, fps: int = 6):
    ...
    for ax, title in zip(axes, titles):
        ...
        ims.append(im)
    fig.suptitle(f"Model 1 rollout: {name}  (max cell diff over {steps} steps: {max(n_diff)})")
...
def save_equivariance_gif(name: str, top: int, left: int, steps: int, r_idx: int = 1, fps: int = 6):
    ...
    at t=0, both rolled out with the model. If equivariance holds exactly,
    the right panel should look like the left panel rotated at every frame."""
    model = load_model()
    ...
    # verify: pred_b[t] should equal torch_d4(pred_a[t], r_idx) exactly
    max_err = 0
    ...
    fig.suptitle(f"Model 1 D4 equivariance: both are pure model rollouts "
...
if __name__ == "__main__":
    ...
    save_equivariance_gif("glider", 10, 10, steps=30, r_idx=1)   # r90
    save_equivariance_gif("beacon", 15, 15, steps=20, r_idx=5)   # flip_h
