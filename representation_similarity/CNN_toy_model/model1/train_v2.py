"""Model 1, v2 training: fix the two gaps found after the v1 run --

1. Density generalization gap (notes.md): v1 trained only on single-pattern
   grids, so recall degraded with density (missed births/survivals in
   neighbor-count regimes v1 never saw). Fix: make_mixed_batch mixes in
   random-density grids (data.py).

2. Autoregressive rollout divergence on dense grids: even a good 1-step
   model can diverge under its own rollout (GoL is chaotic). Fix: scheduled
   sampling (Bengio et al. 2015) -- K=3 unrolled steps, next input is the
   model's own (hard, detached) prediction with probability p or the true
   next state with probability 1-p, p ramped 0->1 over training. Gradient
   flows only through the current step's logits, never through the input-
   selection decision (matches the approach that worked for the V7
   CNN-transformer in ../../../nn/train_cnn_transformer_v7.py, and the
   "why STE doesn't but scheduled sampling does" reasoning in
   [[feedback-gol-nn-training-stability]]).

Warmstarts from v1's checkpoints/best.pt with a much smaller LR, so it's a
fine-tune, not from-scratch training.

Run:
    python train_v2.py --trial
    python train_v2.py --epochs 60
"""
...
from data import make_mixed_batch, make_batch, make_random_batch, make_fixed_set  # noqa: E402
...
def true_trajectory(x0: torch.Tensor, k: int) -> list[torch.Tensor]:
    """[y1, ..., yk]: ground-truth GoL states, purely from x0 (independent
    of what the model predicts -- this is the fixed target at each step)."""
    ys = []
    cur = x0
    for _ in range(k):
...
def evaluate_rollout(model, x0: torch.Tensor, steps: int) -> float:
    """Mean cell-diff count at the final step of an autoregressive rollout,
    averaged over the batch -- tracks whether scheduled sampling is actually
    reducing compounding rollout error, not just 1-step accuracy."""
...
def main():
    ...
    ap.add_argument("--warmup-epochs", type=int, default=40, help="epochs to ramp p 0->1")
    ap.add_argument("--random-frac", type=float, default=0.5)
    ap.add_argument("--m", type=int, default=4)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--warmstart", default=os.path.join(CKPT, "best.pt"))
    args = ap.parse_args()

    device = "cpu"  # see notes.md / train.py: canonical_gidx's CPU round-trip makes MPS slower here
    ...
    val_x, val_y = make_fixed_set(1024, seed=999, device=device)          # single-pattern, matches v1
    ...
    log_path = os.path.join(RESULTS, "train_v2_log.txt")
    hist = []
    best_score = -1e9
    ...
    for ep in range(1, args.epochs + 1):
        ...
        # among pattern-F1-passing epochs, minimise rollout divergence + maximise random recall
        ...
        if score > best_score:
            ...
            torch.save(dict(model=model.state_dict(), epoch=ep, metrics=row, args=vars(args)),
                       os.path.join(CKPT, "best_v2.pt"))
        with open(os.path.join(RESULTS, "metrics_v2.json"), "w") as f:
            json.dump(hist, f, indent=2)

    torch.save(dict(model=model.state_dict(), epoch=args.epochs, metrics=hist[-1], args=vars(args)),
               os.path.join(CKPT, "final_v2.pt"))
    print(f"best score={best_score:.4f}  -> {CKPT}/best_v2.pt  (final also saved)")
