...
two models of different capacity (V8, V10), finding that the larger
model's embeddings are consistently less invariant to all three
transform types.
\end{abstract}
...
different capacity, V8 (291K params) and V10 (1.5M params): for three
base configurations (glider, and two random grids at densities $0.35$ and
$0.65$) we apply rotation ($90^\circ/180^\circ/270^\circ$), reflection
(horizontal/vertical), and translation (1--4 cells), and measure mean
post-Transformer cosine similarity between each grid's embedding and its
transformed counterpart's. Higher similarity means the embedding changed
less under the transform.

V8 shows similarity around $0.88$--$0.89$ for all three transform types;
V10 shows similarity around $0.67$--$0.69$ for all three --- consistently
lower across rotation, reflection, \emph{and} translation alike
(Figure~\ref{fig:pe-verify}, top). Model capacity, not transform type,
is the dominant factor here: the gap between V8 and V10 is roughly
constant ($\approx 0.2$) across all three transforms, while the gap
between transform types within either model is small ($<0.01$ for V8,
$<0.02$ for V10). This is consistent with V10 having learned more
content-sensitive, less redundant embeddings overall as a side effect of
reaching much higher task accuracy (Section~\ref{sec:evolution}), rather
than the larger model specifically losing geometric structure.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/v8_v10_transform_equivariance.png}\\[0.5em]
...
rotation, reflection, and translation, for V8 vs.\ V10; higher means the
embedding changed less under the transform. Bottom: validation accuracy
over training, 10 seeds per activation, on a minimal 25/34-parameter
CNN; polynomial activation converges reliably where ReLU does not.}
\label{fig:pe-verify}
\end{figure}
