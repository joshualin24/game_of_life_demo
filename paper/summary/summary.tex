...
train this same V4 architecture under different regimes and sizes; V2
reappears in Section~\ref{sec:embed} as the point of comparison for
positional-embedding geometry.
...
theoretical prediction (Figure~\ref{fig:v2v8-verify}, top). We note this
is not a fully isolated ablation of positional encoding alone: V2 and V8
sit three architectural changes apart in the V1--V4 progression
(Section~\ref{sec:evolution}), and besides the positional embedding
itself, V2 uses avg-pooled tokenization and zero-padding versus V8's
flatten-and-project tokenization and circular padding. Either confound
could independently affect translation-sensitivity; disentangling them
would require a matched ablation that varies only the positional
embedding on an otherwise-fixed V4 backbone, which we have not yet run.
...
toroidal GoL setup (Figure~\ref{fig:v2v8-verify}, bottom). We then
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/v2_v8_equivariance.png}\\[0.5em]
\includegraphics[width=0.75\linewidth]{figures/relu_vs_poly_convergence.png}
\caption{Top: post-Transformer embedding cosine similarity under
translation, sinusoidal PE (V2) vs.\ learned PE (V8); higher means less
change under translation, i.e.\ more equivariant. Bottom: validation
