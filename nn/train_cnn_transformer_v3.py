"""
Task 11: CNN-Transformer V3 (lossless flatten+linear tokenization) next-step predictor
----------------------------------------------------
Trains a CNNTransformer to predict the GoL state at t+1 given t.

Architecture:
  Stage 1  CNN local encoder (RF=5×5, GroupNorm) → (B, d_model, H, W) feature map
  Stage 2  Flatten patches → Linear(d_model·p², d_model) → (B, 100, d_model) tokens
           No avg_pool: all CNN features within each patch are preserved.
...
Outputs  :
  nn/checkpoints/task11_cnn_transformer_v3_best.pt
  nn/results/task11_cnn_transformer_v3_loss.png
  nn/results/task11_cnn_transformer_v3_rollout_random.png
  nn/results/task11_cnn_transformer_v3_rollout_glider.png
"""
...
from nn.models   import CNNTransformerV3 as CNNTransformer
...
LR         = 3e-4
SEED       = 42
TASK       = "task11_cnn_transformer_v3"
